# Adapted from flash-attention-dao/tests/test_flash_attn.py
import math
...
if TEST_DEVICE == "cuda":
    ...
    apply_rotary_emb = None  # temporally unused
    ...
    from flash_attn_npu.flash_attn_interface import _get_block_size_n
    apply_rotary_emb = None  # temporally unused
else:
...
if TEST_DEVICE == "cuda":
    is_sm75 = torch.cuda.get_device_capability(TEST_DEVICE) == (7, 5)  # Turing
    is_sm8x = torch.cuda.get_device_capability(TEST_DEVICE)[0] == 8 # Ampere
    is_sm80 = torch.cuda.get_device_capability(TEST_DEVICE) == (8, 0)  # Ampere A100
    is_sm90 = torch.cuda.get_device_capability(TEST_DEVICE) == (9, 0)  # Hopper
else:
    is_sm75 = False
    is_sm8x = False
    is_sm80 = False
...
def generate_sparsity_mask(seqlen, sparsity=0.3):
    ...
    mask = torch.rand(nrow, ncol, device=TEST_DEVICE) < sparsity
    return mask
...
def test_flash_attn_qkvpacked(seqlen, d, dropout_p, causal, local, alibi, deterministic, dtype):
    # if seqlen >= 2048 and torch.cuda.get_device_properties(TEST_DEVICE).total_memory <= 16 * 2**30:
...
def test_flash_attn_output(
    ...
    #     and torch.cuda.get_device_properties(TEST_DEVICE).total_memory <= 16 * 2**30
    # ):
    #     pytest.skip()  # Reference implementation OOM
